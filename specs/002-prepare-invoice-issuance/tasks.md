...
- [ ] T008 [P] Establish narrowly shared and test-evidence null-marked boundaries—without a generic request framework—in `src/main/java/com/alexastudillo/taxdocument/api/requestcontext/package-info.java`, `src/main/java/com/alexastudillo/taxdocument/api/problem/package-info.java`, `src/main/java/com/alexastudillo/taxdocument/application/requestcontext/package-info.java`, `src/main/java/com/alexastudillo/taxdocument/infrastructure/requestcontext/package-info.java`, `src/main/java/com/alexastudillo/taxdocument/infrastructure/persistence/package-info.java`, `src/test/java/com/alexastudillo/taxdocument/architecture/package-info.java`, `src/test/java/com/alexastudillo/taxdocument/support/fiscalpreparation/package-info.java`, and `src/integrationTest/java/com/alexastudillo/taxdocument/runtime/package-info.java` for Plan §Structure Decision/CHK022/CHK090
...
- [ ] T013 [P] Replace Feature 001 runtime-OpenAPI byte equality with semantic preservation assertions that tolerate the one approved Feature 002 path while preserving every Feature 001 operation/schema/error and security absence in `src/test/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftOpenApiContractTest.java` for Plan §Source Conflicts and Resolutions/CHK086
...
- [ ] T015 Relocate the safe RFC 9457 transport representation and API-only exception carrier from `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/ProblemDetails.java` into `src/main/java/com/alexastudillo/taxdocument/api/problem/ProblemDetails.java`, update all Feature 001 API consumers, and keep status/error selection capability-specific in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftExceptionMapper.java` for FR-065–FR-067 and Plan §Structure Decision
...
- [ ] T017 Refactor Feature 001 consumers to the shared header, correlation, Problem Details, clock, and deadline types without changing its route, Company/idempotency rules, terminal arbitration, timestamps, or errors in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestBoundary.java`, `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestGateFilter.java`, `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestDeadlineHandler.java`, `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftResource.java`, and `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/CreateInvoiceDraftCommand.java` for Plan §Structure Decision/CHK022
- [ ] T018 Relocate the injectable UTC clock and remaining-budget clamp from `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/SystemRequestClock.java` and `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/ReactiveOperationBudget.java` into `src/main/java/com/alexastudillo/taxdocument/infrastructure/requestcontext/SystemRequestClock.java` and `src/main/java/com/alexastudillo/taxdocument/infrastructure/persistence/ReactiveOperationBudget.java`, update `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftRepositoryAdapter.java`, `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/ReferenceDataRepositoryAdapter.java`, and `src/test/java/com/alexastudillo/taxdocument/support/FixedRequestClock.java`, and remove obsolete copies only after Feature 001 behavior remains green for FR-068–FR-072/FR-081
...
- [ ] T038 [P] [US1] Add capability-level dependency, `@NullMarked`, no-suppression, and exclusion tests proving pure Domain, one real provider port/store/generator boundary, no PostgreSQL sequence, and no auth/Company master/admin/baseline admin/XML/SRI/certificate/PDF/storage/messaging/background implementation in `src/test/java/com/alexastudillo/taxdocument/architecture/FiscalPreparationBoundaryTest.java` for FR-015–FR-016/FR-076–FR-081/SC-013/SC-016
- [ ] T039 [P] [US1] Extend health evidence so liveness remains process-only and readiness requires the same PostgreSQL/Flyway V6 destination but never calls the provider, requires a Company/baseline row, or mutates state in `src/test/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftHealthTest.java` for FR-022/FR-031 and Plan §Liveness and Readiness
- [ ] T040 [P] [US1] Add packaged-JVM first-preparation, replay during provider outage, response-loss recovery, provider timeout-before-baseline, rollback, commit-uncertainty, sanitized telemetry, and unchanged-draft smoke scenarios in `src/integrationTest/java/com/alexastudillo/taxdocument/runtime/FiscalPreparationJvmSmokeIT.java` for SC-001/SC-007–SC-015
- [ ] T041 [P] [US1] Add packaged-JVM 100-same-draft and 100-one-scope-drafts load, event-loop/blocking inspection, pool-20 queueing, deadline-clamp, and post-load recovery evidence in `src/integrationTest/java/com/alexastudillo/taxdocument/runtime/FiscalPreparationJvmPerformanceIT.java` for SC-003–SC-004 and Plan §Performance Goals
...
- [ ] T070 [US1] Update existing service health checks to recognize successful Flyway V6 on the configured PostgreSQL destination while retaining process-only liveness, bounded read-only readiness, and zero provider, Company-specific, baseline-row, or mutation probe in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftLivenessCheck.java` and `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftReadinessCheck.java` for FR-022/FR-031 and Plan §Liveness and Readiness
