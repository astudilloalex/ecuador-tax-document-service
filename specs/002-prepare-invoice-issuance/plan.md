...
src/integrationTest/java/com/alexastudillo/taxdocument/runtime/
...
capability-specific. Architecture and Feature 001 semantic regression tests protect the extraction.
...
| JSpecify 1.0.0 production annotations, Error Prone plugin 5.1.0, Error Prone 2.50.0, NullAway 0.13.7 | FR-081 / SC-016 and explicit owner direction | `-Xlint` only; annotations only; broad suppression | Neither `-Xlint` nor annotations alone performs enforced null analysis | Null-marked new/changed packages, warnings as errors, no suppression escape; build-time cost only |
