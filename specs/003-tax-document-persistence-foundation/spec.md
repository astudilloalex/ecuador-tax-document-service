...
- Q: Should migration failure, rollback, or persisted data repair workflows be task-generated by this feature? → A: No. This feature defines versioned schema artifacts and validation expectations only; migration failure handling, rollback playbooks, and persisted data repair workflows are deferred to a future operations or migration specification.
- Q: Should archive, purge, delete, production data correction, or lifecycle update behavior be task-generated by this feature? → A: No. This feature defines restrictive relationships and common persistence operations only; archive, purge, delete, production correction, and lifecycle update behavior beyond required save/rehydration are deferred.
...
- XML path or audit-event storage is requested before its persistence ownership
  is validated.
- Migration failure handling, rollback playbooks, or persisted data repair
  workflows are requested before a dedicated operations or migration
  specification owns them.
- Archive, purge, delete, production data correction, or lifecycle update
  behavior beyond required persistence and restrictive relationships is
  requested before a dedicated lifecycle or operations specification owns it.
...
- Migration failure handling, rollback playbooks, persisted data repair
  workflows, archival workflows, purge workflows, delete operations,
  production data correction, and lifecycle update behavior beyond required
  `TaxDocumentRepository.save` and rehydration semantics.
...
- **FR-006**: The feature MUST support rehydrating a persisted `TaxDocument`
  ...
  `SequenceNumber`. The restore mechanism is limited to framework-free
  rehydration of common tax document state and MUST NOT introduce
  invoice-specific, credit-note-specific, debit-note-specific,
  withholding-specific, waybill-specific, SRI, XML, queue, webhook, archive,
...
- **FR-015**: The persistence foundation MUST update durable migration
  ...
  include target tables, target columns, primary keys, foreign keys, unique
  constraints, important indexes, temporal mappings, classification as Target
  database objects, and PFV references for deferred compatibility views, XML
  path storage, audit persistence, production data migration, auto-numbering,
...
- **FR-023**: Every future task generated from this specification MUST trace to
  at least one SPEC 003 functional requirement, architectural requirement,
  naming requirement, success criterion, contract, plan section, or data-model
  section. Tasks MUST NOT be generated from implied implementation intent, and
...
- **AR-010**: The feature MUST remain a persistence foundation and MUST NOT
  ...
  configuration. It MUST NOT create `adapter.in.rest`, `adapter.out.sri`,
  `adapter.out.storage`, `adapter.out.queue`, `adapter.out.webhook`, or
...
| migration failure, rollback, and repair workflows | operational migration recovery behavior | Pending Functional Validation | Deferred by PFV-PER-006 |
| archive, purge, delete, and production correction workflows | persistence lifecycle correction behavior | Pending Functional Validation | Deferred by PFV-PER-007 |
| production data migration | legacy-to-target data movement | Pending Functional Validation | Deferred by PFV-PER-008 |
...
| PFV-PER-006 | Migration failure, rollback, and repair | Should migration failure handling, rollback playbooks, or persisted data repair workflows be defined now? | Deferred to a future operations or migration specification. This feature must not generate tasks for rollback playbooks, repair tooling, or recovery operations beyond versioned schema artifacts and validation documentation. |
| PFV-PER-007 | Archive, purge, delete, and production correction | Should archive, purge, delete, production data correction, or lifecycle update behavior be implemented now? | Deferred to a future lifecycle, retention, or operations specification. This feature must not generate tasks for archive, purge, delete, or production correction workflows beyond restrictive schema relationship rules. |
| PFV-PER-008 | Production data migration | Should legacy production data be migrated into the target schema in this feature? | Deferred to a future data migration specification. This feature must not generate production data migration tasks, legacy extraction scripts, data cleansing workflows, or cutover procedures. |
...
- **SC-007**: Scope review finds zero REST resources, REST DTOs, SRI adapters,
  XML generation artifacts, XML storage adapters, queue adapters, webhook
  adapters, bootstrap runtime wiring, archive/purge/delete workflows,
  production correction workflows, migration rollback/repair workflows,
  automatic numbering behavior, or document-specific issuance flows introduced
  by this feature.
...
- Existing application ports from `002-tax-document-issuance-foundation` remain
  the application boundary for SPEC 003. This feature clarifies persistence
  behavior for those ports and does not redesign or rename them.
