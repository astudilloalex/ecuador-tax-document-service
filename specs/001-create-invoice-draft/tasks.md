...
mandatory workflow step is a current `$speckit-analyze`; T017 remains pending, T018 depends on
T017, and T019 remains blocked until both complete successfully.
...
- [X] T010 [P] Add empty-database Flyway, exact locale-independent ASCII product/buyer constraints, aggregate/binding Company enforcement, global reference-catalog no-Company-column checks, prohibited-structure, and repeatability tests for FR-007/FR-010/FR-020–FR-024/FR-030/FR-036–FR-039/DR-014/SC-002/SC-017/SC-022–SC-024 in `src/test/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftMigrationTest.java`; retrospective review found that the historical test omits executable product/buyer ASCII vectors, so T018 supersedes that missing evidence after T017 supplies V5
...
**Status**: **RELEASED — APPROVED WITH MANDATORY CORRECTIVE ACTIONS.**

**Approver**: `astudilloalex`

**Released at**: `2026-07-16T03:35:25Z`

The governance release accepts D1 as historical and D2/D3 only with T017/T018 correction. It
authorizes only progression to T017 after the mandatory current `$speckit-analyze` gate. It does
not authorize skipping T017/T018, starting T019, marking corrective work complete, or executing
later business implementation prematurely. Failure of T017 or T018 blocks all later work.

### Mandatory Corrective Database Evidence

- [ ] T017 Create immutable corrective migration `src/main/resources/db/migration/V5__tighten_invoice_draft_ascii_constraints.sql`; inspect and replace only V3's `ck_invoice_draft_buyer_identification` and `ck_invoice_line_product_code`, reproduce verbatim `^[A-Za-z0-9]{1,20}$` conditionally for buyer types `06`/`08` and `^[A-Za-z0-9]{1,25}$` for product codes using explicit `A-Z`, `a-z`, and `0-9`, never `[[:alnum:]]`, `\w`, locale-sensitive classes, or Unicode shorthand; preserve all other schema behavior, reject invalid buyer/product vectors, leave V3 unchanged, execute through PostgreSQL Flyway, and rely on transactional-DDL failure rollback where PostgreSQL supports it for FR-007/FR-010/FR-020/DR-014/SC-002/SC-010/SC-016/SC-022
- [ ] T018 Depends on T017. Add Flyway/PostgreSQL integration and cross-layer contract evidence in `src/test/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftAsciiConstraintMigrationTest.java` using one shared approved vector table that verifies product code `^[A-Za-z0-9]{1,25}$` and type-conditional passport/foreign identification `^[A-Za-z0-9]{1,20}$` identically through canonical OpenAPI patterns, Java validation, and PostgreSQL constraints; cover lower/upper valid lengths, every uppercase/lowercase/digit category, accented Unicode rejection, U+0020/HTAB/CR/LF/NBSP rejection when not permitted, punctuation acceptance/rejection exactly as each regex defines, empty and over-length values; prove V3 followed by V5 yields the expected final definitions/behavior, update every existing four-migration assertion to five without weakening reference-baseline checks, and prove empty-database migration and Flyway validation succeed for FR-007/FR-010/DR-014/SC-010/SC-016/SC-022
...
- [ ] T022 [P] [US1] Add all five approved buyer strategies; Stage-10 required/type/syntax checks; exact case-sensitive ASCII `^[A-Za-z0-9]{1,20}$` passport/foreign-identification vectors over already normalized values; and separately prove that the final-consumer USD 50.00 check occurs only in Stage 11B after Stage 11A supplies the calculated invoice total, with unsupported-type rejection and no checksum/registry lookup for FR-007/FR-028/FR-041/DR-001/DR-013–DR-015/SC-010/SC-026 in `src/test/java/com/alexastudillo/taxdocument/domain/invoicedraft/BuyerTest.java`
...
- [ ] T026 [P] [US1] Add domain-only tests over already normalized values for Unicode-code-point length, nonblank text, case-sensitive display preservation, email, telephone, collection limits, persisted `canonicalName`, and exact ASCII product-code `^[A-Za-z0-9]{1,25}$`; use shared vectors proving NFC accented names and emoji (`So`) accepted within field limits, decomposed/composed equivalence supplied as NFC, leading/trailing U+0020 removed, internal U+0020 preserved for display, and tab/newline/NBSP/U+2028/U+2029/zero-width `Cf` rejected before domain entry; prove canonical names collapse U+0020 runs and lowercase with `Locale.ROOT` for FR-008–FR-010/FR-015/FR-035/DR-019/SC-015–SC-016 in `src/test/java/com/alexastudillo/taxdocument/domain/invoicedraft/DraftTextRulesTest.java`
- [ ] T027 [P] [US1] Add one-instant America/Guayaquil request-date, impossible/different-date, midnight, rollback non-exposure, immutable replay, and later-date replay tests; prove T063 never invokes the persistence clock or supplies/replaces `createdAt`, while T036 repository-adapter evidence proves T076 owns the single transactional invocation through a resettable test-only fixed clock for FR-006/FR-019/FR-033/DR-012–DR-013/SC-013/SC-030 in `src/test/java/com/alexastudillo/taxdocument/application/invoicedraft/RequestDatePolicyTest.java`
...
- [ ] T029 [P] [US1] Add transport-neutral application use-case tests beginning at FR-041 stage 6 with mapped CompanyId, fixed API-captured request instant, and neutral T054 deadline context; prove NFC/text normalization and prohibited-code-point rejection, Stage 10 independent validation, Stage 11A calculation, deterministic Stage 11B ordering, replay/conflict, persistence handoff, cooperative remaining-budget checks, confirmed pre-persistence zero state, unresolved-commit same-scope replay, zero external calls, no HTTP status/envelope/terminal arbitration, and no persistence-clock invocation by T063 for FR-006–FR-021/FR-024–FR-038/FR-041/FR-043–FR-047/DR-012–DR-013/DR-019/SC-001–SC-002/SC-012/SC-016/SC-023/SC-026/SC-028 in `src/test/java/com/alexastudillo/taxdocument/application/invoicedraft/CreateInvoiceDraftUseCaseTest.java`
- [ ] T030 [P] [US1] Add OpenAPI parse/reference, operation, mandatory exactly-one Idempotency-Key documentation/stable errors, strict request/input Company exclusion with contract-required response `companyId`, exact product/buyer ASCII and general Unicode/canonicalName expectations, payment `emissionDate` effectiveness, Stage 10/11A/11B and API-only deadline/HTTP ownership, decimal, T076-produced `createdAt`, response/error/security absence, `mp.openapi.scan.disable=true`, and byte/semantic equality tests between canonical `specs/001-create-invoice-draft/contracts/invoice-draft-api.openapi.yaml` and runtime publication `src/main/resources/META-INF/openapi.yaml` for FR-001–FR-005/FR-013/FR-019/FR-022/FR-025–FR-027/FR-035/FR-039/FR-041–FR-044/SC-016/SC-020/SC-024–SC-027/SC-032 in `src/test/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftOpenApiContractTest.java`
...
- [ ] T033 [P] [US1] Add API contract tests for the exact Idempotency-Key errors and one-time SP/HTAB normalization; payload-size and strict Company/input rules; exact product/buyer ASCII repertoires; and the general single-line Unicode policy using identical vectors for accented Latin, decomposed versus NFC-composed text, leading/trailing and repeated internal U+0020, tab, newline, NBSP, U+2028/U+2029, zero-width `Cf`, accepted emoji `So`, case differences, and Unicode-code-point length boundaries. Prove NFC occurs once, only internal U+0020 is accepted whitespace, display punctuation/case is preserved, Company identifiers are rejected from request bodies/input schemas, and Stage-5 representation errors are arbitrated only by the API for FR-002/FR-005/FR-007–FR-015/FR-027/FR-035/FR-041–FR-044/DR-019/SC-012/SC-014–SC-016/SC-024–SC-027/SC-029/SC-033 in `src/test/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestValidationTest.java`
- [ ] T034 [P] [US1] Add HTTP 201/200 create/replay and pairwise/multi-failure evidence proving the API adapter is the exclusive request-deadline racer, terminal-result arbiter, HTTP-status selector, Problem Details mapper, and second-response guard: race the application `Uni` against T054 without real sleeps; cover all FR-041 stages including Stage 11A and the exact Stage 11B order; timeout-first versus payload/header/replay/conflict/validation/calculation/persistence outcomes; discard late application/database results; map 504 only in API; map Company 400 and every other HTTP error only after arbitration; preserve a pre-expiry selected result; and make post-response-commit expiry telemetry-only for FR-017–FR-026/FR-030–FR-033/FR-041–FR-043/SC-001–SC-008/SC-012/SC-026–SC-028/SC-033 in `src/test/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftResourceTest.java`
...
- [ ] T036 [P] [US1] Add real-PostgreSQL aggregate round-trip; authoritative Company enforcement for the aggregate/binding and no Company scope on global catalogs; V5 exact ASCII barriers; numeric/canonical-text constraints; and all-or-nothing transaction tests. Prove T076 is the sole transactional clock owner, invokes T059 exactly once after all validations and immediately before root persistence, writes the same returned Instant to root/binding records, exposes none on rollback, never makes a second call in one attempt, and replay returns the persisted value for FR-007/FR-010/FR-019–FR-024/FR-030/FR-037/FR-044/DR-012–DR-014/DR-019/SC-001–SC-002/SC-012/SC-013/SC-017/SC-022/SC-029 in `src/test/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftRepositoryAdapterTest.java`
...
- [ ] T042 [P] [US1] Add packaged JVM boot, migration, served `/q/openapi` resolution and semantic equality/security-absence checks, health, create, replay, conflict, correlation, rollback, timeout, date, and monetary smoke evidence for Constitution III/XII and SC-001–SC-033 in `src/test/java/com/alexastudillo/taxdocument/runtime/InvoiceDraftJvmSmokeTest.java`
...
- [ ] T045 [P] [US1] Implement approved buyer type, exact case-sensitive ASCII `^[A-Za-z0-9]{1,20}$` passport/foreign-identification values after normalization, numeric format-only, contact, and final-consumer domain invariants for FR-007–FR-008/FR-028/DR-014–DR-015 in `src/main/java/com/alexastudillo/taxdocument/domain/invoicedraft/Buyer.java`
...
- [ ] T050 [US1] Implement ordered line inputs, exact case-sensitive ASCII `^[A-Za-z0-9]{1,25}$` product codes after normalization, one tax selection, normalized description, basic quantity/unit-price/discount scale/sign/range inputs, and storage of Stage 11A calculated amounts. Do not perform discount-over-calculated-gross validation here before Stage 11A; T052 owns that ordered Stage 11B check for FR-009–FR-012/FR-041/DR-002–DR-005/DR-010 in `src/main/java/com/alexastudillo/taxdocument/domain/invoicedraft/InvoiceLine.java`
...
- [ ] T053 [US1] Implement the USD DRAFT aggregate with immutable CompanyId, opaque emission point, buyer, ordered local child relationships, taxes, payments, additional information, totals, an already supplied immutable `createdAt` Instant, invalid-relationship rejection, and no fiscal snapshot for FR-004–FR-024/FR-037–FR-038/DR-012–DR-013/DR-020/DR-023 in `src/main/java/com/alexastudillo/taxdocument/domain/invoicedraft/InvoiceDraft.java`

### Application Implementation for User Story 1

- [ ] T054 [P] [US1] Define immutable transport-neutral monotonic start/expiry/remaining-budget context and a controllable time source in `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/RequestDeadline.java`; it contains no terminal-result state, HTTP status, response envelope, timer, or response-commit behavior. Define `CreateInvoiceDraftCommand` carrying that context only for cooperative cancellation/budget checks, mapped CompanyId, one API-captured request instant, idempotency/correlation evidence, and commercial inputs awaiting the one application text-normalization pass for FR-004–FR-015/FR-026–FR-029/FR-040–FR-043 in `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/CreateInvoiceDraftCommand.java`
- [ ] T055 [P] [US1] Define the complete new-or-replayed application result carrying the same persisted immutable `createdAt` required by FR-017–FR-022/SC-001/SC-007/SC-012 in `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/CreateInvoiceDraftResult.java`
...
- [ ] T063 [US1] Implement transport-neutral FR-041 stages 6–12: perform the single general-text NFC/U+0020/prohibited-code-point normalization and `canonicalName` derivation; Company-scoped binding lookup; Stage 10 independent validation; Stage 11A calculation; exact Stage 11B validation; and persistence delegation. Before asynchronous work, cooperatively check T054 and propagate remaining `Duration`, but return only typed application outcomes/failures—never HTTP status, exception, envelope, 504 mapping, terminal arbitration, or second-response logic. T063 MUST NOT invoke T059 persistence time, calculate/replace/overwrite `createdAt`, or accept a persistence timestamp; T076 returns the persisted value. Recover uniqueness through neutral outcomes and resolve uncertain commit only by same-scope replay for FR-006–FR-021/FR-024–FR-038/FR-041/FR-043–FR-047/DR-012–DR-013/DR-019 in `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/CreateInvoiceDraftService.java`
...
- [ ] T073 [P] [US1] Implement the Company/key hash, request fingerprint, version, draft, and binding `createdAt` persistence model using the same immutable transaction-captured value as the bound draft for FR-019/FR-027–FR-034/DR-012 in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftIdempotencyEntity.java`
- [ ] T074 [US1] Implement explicit domain-to-Panache and Panache-to-domain aggregate mappings without leaking persistence types for Constitution IV/FR-020–FR-024/FR-037 in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftPersistenceMapper.java`
...
- [ ] T076 [P] [US1] Implement Company-enforced aggregate/binding reads and mutations while global SRI catalogs remain outside Company scope. T076 is the one and only transactional `createdAt` owner: after all validations succeed, inside the active write transaction and immediately before root persistence, invoke T059 persistence time exactly once; write that same Instant to every record requiring the creation timestamp; return it unchanged; expose none on rollback; never call twice in one attempt; and return the originally persisted value on replay. Handle unique races, budget clamping, unresolved commit, and late completion through typed transport-neutral results only—no HTTP status, 503/504 mapping, response envelope, or terminal arbitration—for FR-019–FR-024/FR-027–FR-034/FR-041/FR-043/DR-012–DR-013 in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftRepositoryAdapter.java`
- [ ] T077 [P] [US1] Implement the default injectable system clock adapter returning `java.time.Instant` through distinct request and persistence operations; API owns the one request-time invocation and T076 owns the one persistence-time invocation. The adapter has no orchestration or call-count ownership, does not query or reconstruct after commit, does not use `track_commit_timestamp`, and does not label either value a physical commit timestamp. Add a resettable deterministic fixed-clock test alternative with invocation counters and no production switch for FR-006/FR-019/DR-012 in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/SystemRequestClock.java` and `src/test/java/com/alexastudillo/taxdocument/support/FixedRequestClock.java`
...
- [ ] T079 [P] [US1] Implement strict client-controlled DTOs with no Company identifier; exact product/buyer ASCII rules; and general single-line Unicode inputs documented as NFC-once, U+0020-only whitespace, Unicode-code-point bounded, case-preserving values with categories `Cc`, `Cf`, `Cs`, `Co`, `Cn`, U+2028, and U+2029 prohibited. Decode only at FR-041 stage 5 and leave the single normalization/canonicalization pass to the API/application boundary before Stage 6 for FR-004–FR-015/FR-035/FR-041–FR-042/FR-044/DR-019 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/CreateInvoiceDraftRequest.java`; FR-027 remains exclusively in the API header adapter
...
**Checkpoint**: User Story 1 independently satisfies all 68 acceptance scenarios, all 33 success
...
- [ ] T096 Validate and record all approved end-to-end, recovery, exact header-cardinality/error, V3→V5 ASCII, Unicode/canonicalName, Stage 10/11A/11B, payment-emissionDate, API-only deadline, T076-only transactional `createdAt`, Company request/response, repository/global-catalog, negative-architecture, and dynamic America/Guayaquil date scenarios in `specs/001-create-invoice-draft/quickstart.md`
...
- [ ] T099 Record the final Constitution v2.0.1 and Definition of Done review, including `GOV-001` disposition, Company request/input exclusion and contract-required response, Company-owned aggregate/binding scope with unscoped global catalogs, no identity/Company dependency/snapshot/fiscal side effect, V5 migration, Unicode policy, payment effectivity, API-only deadline arbitration, single T076 clock ownership, sensitive-data, and runtime evidence in `specs/001-create-invoice-draft/plan.md`
- [ ] T100 Reconcile final FR-001–FR-047, DR-001–DR-024, SC-001–SC-033, AS-001–AS-068, stable-error, governance-gate, and prohibited-boundary evidence in `specs/001-create-invoice-draft/traceability.md`
...
Constitution v2.0.1 amendment APPROVED
  ...
  → current $speckit-analyze with no related CRITICAL finding
      → T017 immutable V5 corrective migration
      → T018 corrective integration evidence
...
- Phase 2 depends on Phase 1. T010 and T011 depend on T008; T013–T016 are historical migration work.
  T017 depends on gate release and creates immutable V5 without editing V3; T018 depends on T017
  and proves the corrected final schema plus cross-layer equivalence.
...
- Domain production starts only after its evidence tasks exist. T044–T049 may run in parallel;
  T050–T053 then build line, tax-total, calculation, and aggregate behavior in dependency order.
- Application tasks depend on T053. T054–T055 and T057–T060/T062 may run in parallel; T056/T061
  follow their direct types, and T063 integrates the completed application boundaries.
- T064–T067 depend on the foundational migrations and may run in parallel. T068–T073 depend on
  ...
  T077–T078 depend only on their application ports. T076 depends on T059, T067–T074, and T077 and
  is the sole owner of the one persistence-time clock invocation.
...
- `$speckit-analyze` was required after task generation and before T001, but that sequence was not
  ...
  dispositions are approved and `GATE-GOV-001` is released. A current analysis must now establish
  the no-CRITICAL implementation condition before T017 starts. The Final Phase depends on
  T017–T092 and the released gate.
...
Application ports:    T054 T055 T057 T058 T059 T060 T062
...
batch to be complete. After API foundations, complete T084 and the independently authored T092
telemetry boundary, then T085 before T088; T084 and T085 are intentionally not parallel.
...
2. Preserve the completed `astudilloalex` approval and released `GATE-GOV-001` recorded in
   `governance-owner-approval.md`.
3. Run the mandatory current `$speckit-analyze`; do not begin implementation if it reports a
   CRITICAL finding related to this non-conformity or database evolution.
4. Do not execute `$speckit-implement` or start T017 before that current analysis gate succeeds.
5. Complete T017 then T018 before T019 or any other business task; failure in either blocks all
   later work.
