...
- [X] T010 [P] Add empty-database Flyway, schema constraint, prohibited-structure, and repeatability tests for FR-020–FR-024/FR-030/FR-036–FR-039/SC-002/SC-017/SC-022–SC-024 in `src/test/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftMigrationTest.java`
...
- [X] T013 Create the exact versioned buyer-identification, IVA-rule, and payment-method catalog structures from the approved model in `src/main/resources/db/migration/V1__create_invoice_draft_reference_catalogs.sql`
- [X] T014 Seed exactly the approved SRI-OFFLINE-2.32-TARGET-1 rows and fixed UUIDs with verification queries in `src/main/resources/db/migration/V2__seed_invoice_draft_reference_data.sql`
- [X] T015 Create the Invoice Draft root, line, line-tax, grouped-tax, payment, and additional-information structures and constraints for FR-020–FR-024/FR-037–FR-038/DR-020–DR-023 in `src/main/resources/db/migration/V3__create_invoice_draft_aggregate.sql`
...
- [ ] T085 [US1] Implement exact 400/409/422/500/503/504 safe error mapping, including Jackson/Bean Validation failures after the built-in mismatched-input mapper is disabled, application-detected deadline exhaustion, and correlation headers from the approved catalog; explicitly delegate/exclude 413 because T083 is its sole transport owner, for FR-025–FR-026/FR-030/FR-041–FR-044 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftExceptionMapper.java`
- [ ] T086 [US1] Expose POST `/api/v1/invoice-drafts`, apply T082's `@InvoiceDraftRequestGate` name binding to the resource method, consume the request-local deadline/Company/key/correlation values produced by T082/T083 and the typed request decoded only after FR-041 stage 5, call `RequestClock` exactly once after those stages pass, pass the deadline and instant through T084, keep T083's timer armed through response completion, and implement 201 new, 200 replay, and complete response semantics for FR-001–FR-003/FR-006/FR-017–FR-022/FR-026–FR-033/FR-041/FR-043/DR-012 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftResource.java`
...
- T077–T081 may run in parallel after the application contract is stable. T082 depends on T052 and
  ...
  result mapping begins. T085 depends on T081. T086 depends on T075 and T077–T085 so it receives
