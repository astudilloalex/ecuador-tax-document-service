...
analysis after the approved corrective-assignment addendum found no CRITICAL corrective-assignment
issue; therefore T017 is eligible to begin. T017 remains pending, T018 depends on successful T017,
and T019 remains blocked until both complete successfully.
...
**Status**: **RELEASED — APPROVED WITH MANDATORY CORRECTIVE ACTIONS; SUBSEQUENT ANALYSIS CONDITION
SATISFIED.**
...
- [ ] T020 [P] [US1] Create the one authoritative general-text fixture in `src/test/resources/invoicedraft/unicode-text-validation-vectors.json`; every entry contains `id`, `fieldCategory`, `rawValue`, `applicationNormalizedValue`, `canonicalValue` when applicable, `expectedApplicationOutcome`, `expectedDomainInput`, `expectedStoredValue`, `expectedErrorCode`, `failureStage`, `rationale`, and `consumers`, with `failureStage` exactly `NONE`, `TRANSPORT_REPRESENTATION`, `APPLICATION_STAGE_6`, or `PERSISTENCE_DEFENSE`; dedicated persistence-defense entries also contain `storedProbeValue` and `expectedPersistenceOutcome`. Cover NFC accented Latin, decomposed/composed equivalence, leading/trailing and repeated internal U+0020, tab, CR, LF, NBSP, U+2028, U+2029, zero-width `Cf`, accepted assigned emoji `So`, preserved display case, code-point boundaries, 150 accepted and 151 rejected `U+0130` canonical expansion, `CANONICAL_NAME_TOO_LONG`, and no truncation. Also create normalization-version-1 key-hash/fingerprint vectors in `src/test/resources/invoicedraft/idempotency-v1-vectors.json` for UUID, decimal, null/empty, collection order, Company, key, and correlation rules; those idempotency vectors MUST reference or consume the authoritative Unicode fixture instead of redefining text-normalization cases. T020 owns fixture structure and stage meanings: API uses raw decoding/forwarding only; Application produces normalization/canonical outcomes; Domain receives accepted `expectedDomainInput` only; Infrastructure uses `expectedStoredValue` or dedicated defensive probes only for FR-027–FR-035/DR-017–DR-021/SC-012/SC-016/SC-018–SC-019/SC-030/SC-033
...
- Phase 2 depends on Phase 1. T010 and T011 depend on T008; T013–T016 are historical migration work.
  T017 depends only on the released gate plus the satisfied subsequent-analysis condition and creates shared
...
- US1 depends on Phase 2, released `GATE-GOV-001`, T017, and T018. T019 establishes calculation
  ...
  T029, T030, T033, and T036 depend explicitly on T020; T028 consumes the Unicode fixture
  indirectly through T020. Other T021–T043 evidence tasks may be authored in parallel only after
  their stated fixture dependencies because they change separate test files.
...
- T079–T083 may run in parallel after the application contract is stable. T084 depends on T054 and
  ...
  accepted by T085. T088 depends on T077 and T079–T087 so it receives gated, decoded inputs before
  the one authorized request-time read.
- `$speckit-analyze` was required after task generation and before T001, but that sequence was not
  ...
  also approved, and the subsequent analysis found no CRITICAL corrective-assignment issue. T017 is
  eligible to begin; T018 still requires successful T017, T019 still requires both, and the Final
  Phase depends on T017–T092 and the released gate.
...
3. Preserve the result of the subsequent analysis that found no CRITICAL corrective-assignment
   issue; T017 is therefore eligible to begin.
