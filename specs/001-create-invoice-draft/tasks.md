...
- [ ] T025 [P] [US1] Add one-instant America/Guayaquil current-date, impossible/different-date rejection without normalization, midnight-commit, confirmed-createdAt, and later-date replay tests using a resettable test-only fixed-clock CDI alternative for FR-006/FR-019/FR-033/DR-012–DR-013/SC-013/SC-030 in `src/test/java/com/alexastudillo/taxdocument/application/invoicedraft/RequestDatePolicyTest.java`
...
- [ ] T031 [P] [US1] Add Content-Length and chunked exact 2 MiB/2 MiB+1 payload behavior with absent, valid, and invalid correlation headers, proving over-limit-first stable correlated `413`, valid preservation, invalid safe replacement without `400`, and no deserialization/idempotency/reference/validation/calculation/persistence call; plus idempotency-key grammar, deferred malformed-JSON/Bean Validation, strict unknown/prohibited property, calculated-field, numeric representation, exact 1/8/9 payment collection, text, and other collection contract tests for FR-002/FR-005/FR-008–FR-015/FR-027/FR-035/FR-041–FR-044/SC-014–SC-016/SC-025–SC-027/SC-029/SC-033 in `src/test/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestValidationTest.java`
- [ ] T032 [P] [US1] Add HTTP 201/200 create/replay, complete response mapping, stable Problem Details, and pairwise/multi-failure evidence using a controllable fake monotonic clock or T052 deadline abstraction with no real sleeps. All non-timeout scenario tests MUST configure the controlled request deadline as non-expiring or with sufficient remaining budget so the tested stage conclusively completes before expiry. Deadline-race tests MUST independently control stage completion and deadline expiry and deterministically prove that the first conclusively selected outcome wins: all FR-041 stages; payload-size-first `413` versus deadline-first `504`; header-invalid-first `400` versus deadline-first `504`; replay/conflict, validation/calculation, known persistence outcome, and unresolved-commit races; first-outcome immutability; header-before-entity decoding; safe correlation; one fixed deadline propagation/cancellation; zero state only for confirmed pre-persistence or complete rollback; and post-response-commit expiry that preserves status/body, emits no `504` or second response/database write/compensation, and records `request_deadline_exceeded_after_response_commit` for FR-017–FR-026/FR-030–FR-033/FR-041–FR-043/SC-001–SC-008/SC-012/SC-026–SC-028/SC-033 in `src/test/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftResourceTest.java`
...
- [ ] T055 [P] [US1] Define the minimal Company-scoped local aggregate, binding lookup, and atomic create boundary whose asynchronous operations receive an explicit remaining `Duration` budget for FR-020–FR-024/FR-030–FR-034/FR-043 in `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/InvoiceDraftRepository.java`
...
- [ ] T061 [US1] Implement only FR-041 stages 6–12 from the already mapped command: before each asynchronous stage atomically honor the first conclusive stage/deadline outcome, reject an exhausted deadline without starting new work, propagate the current remaining `Duration` to every aggregate and T056 reference-data call, normalize content, perform Company-scoped replay/conflict and current catalog/domain validation including DR-013 rejection, calculate deterministically, hand off atomically, recover the uniqueness winner, preserve selected HTTP semantics when late completion arrives, and resolve uncertain commit only through same-scope replay for FR-006–FR-021/FR-024–FR-038/FR-041/FR-043–FR-047/DR-013 in `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/CreateInvoiceDraftService.java`
...
- [ ] T074 [P] [US1] Implement reactive Company-scoped binding/root reads, atomic root/children/binding persistence, invalid local-relationship and partial-state rejection, unique-race rollback and winner re-read, no-new-work-after-expiry behavior, pool/query timeout clamping to the supplied remaining budget, and write timeout clamping to the lesser of that budget and five seconds; map confirmed configured-timeout-first/unavailability with remaining request budget to `PERSISTENCE_UNAVAILABLE`, shared-deadline-first expiry to `REQUEST_TIMEOUT`, make no zero-state claim while commit outcome is unresolved at deadline, let no late database completion replace selected `504`, and resolve authoritative state only through same-scope replay without compensation for FR-020–FR-024/FR-027–FR-034/FR-041/FR-043/DR-013 in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftRepositoryAdapter.java`
- [ ] T075 [P] [US1] Implement the default injectable system clock adapter used once by the API for the request instant and by persistence for confirmed commit instants in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/SystemRequestClock.java`, plus a resettable fixed-clock CDI `@Alternative` with test-only `@Priority` activation confined to `src/test/java/com/alexastudillo/taxdocument/support/FixedRequestClock.java`, with no production configuration switch, for FR-006/FR-019/DR-012
...
- [ ] T077 [P] [US1] Implement strict client-controlled create DTOs with approved field, text, numeric-string, exact 1–8 payment and other collection, and prohibited-property semantics, decoded only at FR-041 stage 5 after the request gate passes, for FR-004–FR-015/FR-027/FR-035/FR-042/FR-044 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/CreateInvoiceDraftRequest.java`
...
- T077–T081 may run in parallel after the application contract is stable. T082 depends on T052 and
  T079–T081. T083 and T088–T090 may run in parallel after their configuration and application
  dependencies exist. T083 depends on T052. T084 depends on T052, T081–T083. T085 depends on
  T052–T053 and T077–T078 and is intentionally not parallel; both application results and API
  response contracts must exist before result mapping begins. T086 depends on T081 and T084. T087
  depends on T075 and T077–T086 so it receives gated, decoded inputs before the one authorized
  request-time read.
- `$speckit-analyze` is an artifact gate after task generation and before T001. Once it clears every
  critical finding, `$speckit-implement` executes Phase 1 through the Final Phase. The Final Phase
  depends on all US1 tasks and does not precede the analysis gate.
...
Operational adapters: T083 T088 T089 T090
```

Tasks listed in the same batch modify independent files but still require the earlier dependency
batch to be complete. After API foundations, complete T082 and the independently authored T083
telemetry boundary, then T084 before T087; T082 and T084 are intentionally not parallel.

## Implementation Strategy

### MVP First

1. After task generation, run `$speckit-analyze`; do not start T001 until every critical finding is
   resolved.
2. Run `$speckit-implement`, complete Phase 1, and prove the approved Quarkus/Java build baseline.
3. Complete Phase 2 and prove the exact schema/reference baseline from empty PostgreSQL.
4. Write all US1 evidence tasks before their production tasks.
...
7. Complete Definition of Done evidence; the artifact analysis gate has already run before
   implementation.
