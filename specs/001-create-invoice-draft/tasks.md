...
- [ ] T082 [US1] Implement custom Jakarta REST `@NameBinding` `@InvoiceDraftRequestGate` in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestGate.java` and one name-bound `@ServerRequestFilter(nonBlocking = true)` pre-entity gate in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestGateFilter.java` that uses T079/T080/T081 and evaluates Company, safe correlation generation/preservation/replacement, then idempotency key in exact FR-041 stages 2–4, placing only accepted mapped values in request-local API state for FR-001–FR-003/FR-026–FR-027/FR-040–FR-041/DR-024
- [ ] T083 [US1] Implement a CDI `Router` observer that registers a Vert.x failure handler using T081/T082 for `quarkus.http.limits.max-body-size=2097152`, owns only status 413 for `POST /api/v1/invoice-drafts`, maps every Content-Length or chunked over-limit failure there to the approved correlated 413 without running Company or correlation validity, and delegates every unrelated failure for FR-026/FR-041–FR-042/SC-027 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftPayloadSizeFailureHandler.java`
...
- [ ] T085 [US1] Implement exact 400/409/413/422/500/503/504 safe error mapping, including Jackson/Bean Validation failures after the built-in mismatched-input mapper is disabled, and correlation headers from the approved catalog for FR-025–FR-026/FR-030/FR-041–FR-044 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftExceptionMapper.java`
- [ ] T086 [US1] Expose POST `/api/v1/invoice-drafts`, apply T082's `@InvoiceDraftRequestGate` name binding to the resource method, consume the request-local Company/key/correlation values produced by T082 and the typed request decoded only after FR-041 stage 5, call `RequestClock` exactly once after those stages pass, pass that instant to T084, and implement 201 new, 200 replay, and complete response semantics for FR-001–FR-003/FR-006/FR-017–FR-022/FR-026–FR-033/FR-041/DR-012 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftResource.java`
...
- T077–T081 may run in parallel after the application contract is stable. T082 depends on T079–T081;
  T083 depends on T081–T082. T084 depends on T077; T085 depends on T081;
