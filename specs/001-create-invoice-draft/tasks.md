...
- [ ] T036 [P] [US1] Add injected confirmed pre-commit failure after every write phase plus unavailable and query/write-timeout cases, asserting zero partial state only when pre-commit or full rollback is confirmed; separately simulate uncertain commit and post-commit response loss and prove equivalent replay recovers the authoritative draft for FR-020–FR-021/FR-032/FR-043/DR-013/SC-002/SC-028 in `src/test/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftRollbackTest.java`
...
- [ ] T061 [US1] Implement only FR-041 stages 6–12 from the already mapped command: reject exhausted deadlines before starting another asynchronous stage, propagate the current remaining budget to every repository call, normalize content, perform Company-scoped replay/conflict and current catalog/domain validation including DR-013 rejection, calculate deterministically, hand off atomically, recover the uniqueness winner, and return safe terminal results for FR-006–FR-021/FR-024–FR-038/FR-041/FR-043–FR-047/DR-013 in `src/main/java/com/alexastudillo/taxdocument/application/invoicedraft/CreateInvoiceDraftService.java`
...
- [ ] T083 [US1] Implement the earliest-ordered CDI `Router` acceptance handler for `POST /api/v1/invoice-drafts` in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftRequestDeadlineHandler.java`: before body consumption create T052's fixed monotonic deadline from the configured `PT10S`, initialize T082's safe correlation classification without emitting its stage-3 error, store both in request-local state, arm one non-blocking timer, preserve the first FR-041 terminal outcome, return correlated 504 only while the response is uncommitted, keep the timer through serialization, cancel it on response end, and never compensate a possible commit. In the same task, implement the exclusive status-413 Vert.x failure handler for this operation in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftPayloadSizeFailureHandler.java`; it uses T081/T082 to preserve one valid correlation value or generate a safe replacement for absent/invalid input, never emits Company or correlation-invalid outcomes, maps every Content-Length or chunked over-limit failure to the approved 413, and delegates every unrelated failure for FR-026/FR-041–FR-043/SC-027
- [ ] T084 [US1] Implement explicit request-to-command mapping that carries the one request-local fixed deadline, mapped Company UUID/business inputs, and one API-captured request instant without reading the clock, plus result-to-response mapping, for FR-006/FR-022/FR-037/FR-040/FR-043/DR-012 in `src/main/java/com/alexastudillo/taxdocument/api/invoicedraft/InvoiceDraftApiMapper.java`
...
- [ ] T090 [P] [US1] Implement bounded metrics, safe structured audit/log/trace context, durations from the same earliest-boundary monotonic start used by T083, and new/replay/conflict/rollback/timeout outcomes without sensitive or high-cardinality labels for FR-025–FR-026/FR-043/SC-002/SC-012/SC-023/SC-033 in `src/main/java/com/alexastudillo/taxdocument/infrastructure/invoicedraft/InvoiceDraftTelemetry.java`
...
- [ ] T095 Record the exact JVM/PostgreSQL measurement environment, warm-up, samples, percentiles, resource use, blocked-thread result, earliest-boundary deadline start, remaining-budget clamping, response-end timer cancellation, and pool recovery evidence in `specs/001-create-invoice-draft/operational-requirements.md`
...
- T077–T081 may run in parallel after the application contract is stable. T082 depends on T052 and
  T079–T081; T083 depends on T052 and T081–T082. T084 depends on T052 and T077; T085 depends on T081;
  T086 depends on T075 and T077–T085 so it receives gated, decoded inputs before the one authorized
  request-time read. T087–T090 may run in parallel after their configuration and application
  dependencies exist.
...
batch to be complete. After API foundations, complete the ordered gate sequence T082 → T083 before
T086; those two tasks are intentionally not parallel.
