...
| `FR-008` | OpenAPI exact general Unicode/contact policy; `data-model.md` buyer fields | Scenarios 31–33, 69; `SC-016` | API forwards decoded values; Application invokes one Stage-6 normalizer/value; shared NFC, prohibited-category/separator, U+0020, emoji, case, code-point-boundary, email/telephone vectors; Domain/Infrastructure do not normalize |
...
| `FR-015` | OpenAPI Unicode additional-information schema; persisted Java `canonicalName` | Scenarios 29–33, 69, 70; `SC-015`, `SC-016` | 0/15/16 plus Application-owned exact NFC → U+0020 trim/collapse → Locale.ROOT lowercase → 1–300-code-point validation, `U+0130` expansion, `CANONICAL_NAME_TOO_LONG`, no truncation, canonical uniqueness |
...
| `FR-035` | Exact Application-owned NFC/U+0020/prohibited-category/code-point policy and canonicalName design | Scenarios 31–33, 37, 69, 70; `SC-016`, `SC-019` | API unchanged handoff; one Application normalizer invocation/value; identical accented/decomposed, trim/internal space, tab/newline/NBSP/U+2028/U+2029/Cf, emoji So, case, `U+0130`, display/canonical code-point boundary vectors; no downstream normalization |
...
| `DR-019` | Exact Application-owned general Unicode normalization/canonicalName plus stricter field rules | Scenarios 31–33, 37, 61, 69, 70; `SC-012`, `SC-016`, `SC-019` | API unchanged handoff; one Stage-6 `BusinessTextNormalizer` invocation/value; one NFC pass, prohibited categories/separators, U+0020 only, display/canonical code-point lengths, display case, emoji So, Java Locale.ROOT persisted canonicalName, U+0130 expansion/`CANONICAL_NAME_TOO_LONG`; Domain/Infrastructure do not normalize; strict ASCII and API-only Idempotency-Key remain separate |
...
| `SC-016` | `FR-008`, `FR-010`, `FR-015`, `FR-035`; `DR-019` | API unchanged handoff, exactly one Application normalizer invocation/value, zero Domain/Infrastructure passes, identical exact ASCII/Unicode/canonicalName vectors, U+0130 150/151 boundaries, stable canonical overflow, and invalid-value rejection; header evidence remains in SC-012 |
...
| `AS-069` | `FR-008`, `FR-015`, `FR-035`, `FR-040`, `FR-041`; `DR-019` | API forwards decoded business text unchanged; Application invokes `BusinessTextNormalizer` once per supplied applicable value at Stage 6; Domain/Infrastructure invoke it zero times |
