...
`GATE-GOV-001` is released. The current analysis gate remains before pending T017; pending T018
depends on T017, and T019 remains blocked until both complete successfully.
...
|-------------|----------------------|--------------------|----------------|
| `GOV-001` / `GATE-GOV-001` | Completed retrospective, approved D1–D3 dispositions, and mandatory corrective controls | `governance-retrospective-review.md`; `governance-owner-approval.md`; `governance-nonconformity.md`; commits `1289871`, `8bdd548`, `5e5452a` | `RELEASED`; approved by `astudilloalex`; current analysis remains required |
| `T017` | Create immutable V5 replacing only V3's affected buyer/product ASCII constraints | `FR-007`, `FR-010`, `FR-020`; `DR-014`; `SC-002`, `SC-010`, `SC-016`, `SC-022`; C2/D3 | Pending; governance-authorized after current analysis; V3 remains immutable |
| `T018` | Prove V3→V5, Flyway validation, and identical OpenAPI/Java/PostgreSQL ASCII vectors | `FR-007`, `FR-010`; `DR-014`; `SC-010`, `SC-016`, `SC-022`; C2/D2 | Pending; depends on T017 |
...
| `FR-007` | Approved identification baseline and exact ASCII rule in `reference-data-baseline.md` | Scenarios 7, 9, 13–16, 53; `SC-010`, `SC-031` | T017 V5 plus T018 exact FORMAT_ONLY vectors; code 06/08 case-sensitive `^[A-Za-z0-9]{1,20}$` across OpenAPI/Java/domain/PostgreSQL |
| `FR-008` | OpenAPI exact general Unicode/contact policy; `data-model.md` buyer fields | Scenarios 31–33; `SC-016` | Shared NFC, prohibited-category/separator, U+0020, emoji, case, code-point-boundary, email/telephone vectors |
...
| `FR-010` | OpenAPI line numeric/text bounds; `data-model.md` decimals and exact product repertoire | Scenarios 2, 3, 31, 32, 49, 50; `SC-004`, `SC-016`, `SC-029` | T017 V5/T018 cross-layer ASCII `^[A-Za-z0-9]{1,25}$`; description Unicode; quantity/price/discount precision/range |
...
| `FR-015` | OpenAPI Unicode additional-information schema; persisted Java `canonicalName` | Scenarios 29–33; `SC-015`, `SC-016` | 0/15/16 plus exact NFC → U+0020 trim/collapse → Locale.ROOT canonical uniqueness and shared vectors |
...
| `FR-019` | `data-model.md` `timestamptz`; T076-only transactional clock design | Scenarios 1, 51, 52, 61; `SC-001`, `SC-007`, `SC-012`, `SC-013`, `SC-030` | T076 calls once after validation/immediately before persistence and writes one Instant; T063 never calls/supplies/overwrites; replay/rollback; no physical commit timestamp/post-commit query/`track_commit_timestamp` |
...
| `FR-035` | Exact NFC/U+0020/prohibited-category/code-point policy and canonicalName design | Scenarios 31–33, 37; `SC-016`, `SC-019` | Identical accented/decomposed, trim/internal space, tab/newline/NBSP/U+2028/U+2029/Cf, emoji So, case, and code-point boundary vectors |
...
| `FR-041` | Executable Stage 10/11A/ordered 11B; API-exclusive deadline race/HTTP mapping; neutral application/repositories | Scenarios 3, 5, 15, 45–46, 57–61; `SC-012`, `SC-026`, `SC-033` | T019/T021/T022/T029/T034/T052/T063 prove stage separation/order; T034/T085/T088 prove API Uni race, one response, late discard, API-only 504/all HTTP mapping |
...
| `DR-012` | Separate request-date and T076-only transaction-createdAt clock semantics | Scenarios 26, 27, 51, 52, 61; `SC-012`, `SC-013`, `SC-030` | One request Instant/Guayaquil date; T076 sole once-only clock at pre-persist point; T063 no call/value ownership; same persisted/returned/replayed value; rollback/no physical timestamp |
...
| `DR-019` | Exact general Unicode normalization/canonicalName plus stricter field rules | Scenarios 31–33, 37, 61; `SC-012`, `SC-016`, `SC-019` | One NFC pass, prohibited categories/separators, U+0020 only, code-point length, display case, emoji So, Java Locale.ROOT persisted canonicalName; strict ASCII and API-only Idempotency-Key remain separate |
...
| `SC-013` | `FR-006`, `FR-019`; `DR-012` | Request-date/midnight vectors plus T076 as sole once-only transaction clock owner; T063 no invocation/overwrite; no physical commit timestamp |
...
| `SC-016` | `FR-008`, `FR-010`, `FR-015`, `FR-035`; `DR-019` | Identical exact ASCII and Unicode/canonicalName vectors across applicable layers; invalid values reject; header evidence remains in SC-012 |
