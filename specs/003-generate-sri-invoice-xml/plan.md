...
| FR-001–013 / FR-076–081 API | Quarkus API/REST Assured | `api/invoicexml/InvoiceXmlResourceTest.java`; `InvoiceXmlRequestDeadlineHandlerTest.java` | exact header/path/no-body contract, 201/200, identical bodies, canonical Base64, no-store, safe Problem Details/correlation | missing/repeated/comma/blank/malformed/nil Company, path nil, body/query/Idempotency-Key, deadline race, every error mapping |
| API source of truth | Semantic OpenAPI test | `api/invoicexml/InvoiceXmlOpenApiContractTest.java` and existing Feature 001/002 regressions | standalone contract merges into `META-INF/openapi.yaml` without changing prior routes; no security/Company input/replay flag | 401/403/security scheme, raw XML media type, status field, response Company, missing errors/headers |
